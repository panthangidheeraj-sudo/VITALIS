/**
 * Offline first-aid content (spec §5.8).
 *
 * Bundled into the JS bundle, so it is present on first launch with no network
 * and no sync step. This is the module the spec explicitly separates from the
 * live agentic loop: it is static reference text, it never calls a tool, and it
 * never produces a risk tier.
 *
 * SOURCING: these are condensed from widely-taught lay-rescuer basics (the
 * compression rate and depth figures are the standard AHA/ERC lay-CPR numbers).
 * They are NOT independently clinically reviewed for this project, which is why
 * every topic carries a `disclaimer` that renders on screen, and why the
 * emergency number is shown on every card.
 */
...
export interface FirstAidTopic {
  ...
  /** One line describing when this applies — shown before the steps. */
